...
1. Allow connections from Airbyte to your AWS S3/Minio S3 cluster (if they exist in separate VPCs).
   If you're using Airbyte Cloud and your storage service uses IP-based access controls, add
...
4. Configure fields:
   ...
   - **S3 Filename pattern**
     ...
     placeholders, as they won't be recognized.
   <!-- /env:oss -->
...
deletion due to misconfiguration. This applies to file-transfer ("Copy Raw Files") streams as well as
regular ones: an object without current generation metadata is treated as stale regardless of its
filename, so a stream's S3 path should only ever contain objects this connector wrote. ⚠️
...
[binary encoding](https://avro.apache.org/docs/1.12.0/specification/#binary-encoding), and assumes that
...
|:------------|:-----------|:-----------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------|
| 1.9.11      | 2026-09-23 | [outrauk/airbyte#1](https://github.com/outrauk/airbyte)  | Re-tag 1.9.10 - `tag-and-release`'s needs were being skipped (test-connectors doesn't run for a JVM-only change), which skips a dependent job by default regardless of its own if:, so no tag was cut for it either. |
| 1.9.10      | 2026-09-23 | [outrauk/airbyte#1](https://github.com/outrauk/airbyte)  | Re-tag 1.9.9 - CI's connector-changed filter didn't cover this connector yet when 1.9.9 was merged, so no image/tag was published for it. |
| 1.9.9       | 2026-09-23 | [outrauk/airbyte#1](https://github.com/outrauk/airbyte)  | Fix Overwrite mode not deleting previous sync's files for file-transfer ("Copy Raw Files") streams. |
